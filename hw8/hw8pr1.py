...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
    ...
    while iter_num < max_iter:
        ...
        for i in range(m):
            ...
            dists = np.zeros((k, 1))
            point = X[i, :]
            dists = np.linalg.norm(clusters - point, axis=1).reshape((k, 1))
            label[i] = np.argmin(dists)
        # update clusters
        prev_clusters = copy.deepcopy(clusters)
        for i in range(k):
            # i took the following line from the solutions
            # i had ind = (labels == i).reshape(-1)
...
def k_means_cost(X, clusters, label):
    ...
    "*** YOUR CODE HERE ***"
    for i in range(k):
        cost += np.linalg.norm(X[label.reshape(-1) == i, :] - clusters[i])
    "*** END YOUR CODE HERE ***"
    return cost
...
if __name__ == '__main__':
    ...
    "*** YOUR CODE HERE ***"
    plt.scatter(list(range(1, 21)), cost_k_list)
    ...
    center_plot, = plt.plot(clusters[:, 0], clusters[:, 1], 'bD')
    "*** END YOUR CODE HERE ***"
