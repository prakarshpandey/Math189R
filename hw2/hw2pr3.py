...
def linreg(X, y, reg=0.0):
    ...
    """
    "*** YOUR CODE HERE ***"
    # consulted solution here. i was squaring reg for some reason
    # i was also regularizing the bias term, which was causing issues
    I_D = np.eye(X.shape[1])
    I_D[0, 0] = 0
    W_opt = np.linalg.solve(X.T @ X + reg * I_D, X.T @ y)
    "*** END YOUR CODE HERE ***"
    return W_opt
...
def find_RMSE(W, X, y):
    ...
    "*** YOUR CODE HERE ***"
    preds = predict(W, X)
    RMSE = np.sqrt((np.square(preds - y)).mean())
    "*** END YOUR CODE HERE ***"
    return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
    ...
    # got the following line from the solution. not sure why sorting matters here.
    reg_list.sort()
    W_list = [linreg(X_train, y_train, reg=reg_i) for reg_i in reg_list]
    RMSE_list = []
    for i in range(0, reg_list.size):
        W_i = W_list[i]
        RMSE_list.append(find_RMSE(W_i, X_val, y_val))
    "*** END YOUR CODE HERE ***"
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
    ...
    reg_list.sort()
    W_list = [linreg(X_train, y_train, reg=reg_i) for reg_i in reg_list]
    norm_list = [np.linalg.norm(W_i) for W_i in W_list]
    "*** END YOUR CODE HERE ***"
...
def linreg_no_bias(X, y, reg=0.0):
    ...
    one = np.ones_like(y)
    m = X.shape[0]
    W_opt = np.linalg.solve(X.T@X + reg*np.eye(X.shape[1]) - (X.T @ one @ one.T @ X) / m,
                             X.T @ y - (X.T @ one @ one.T @ y) / m)
    b_opt = (one.T @ y - one.T @ X @ W_opt) / m
    "*** END YOUR CODE HERE ***"
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
    ...
    while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
        ...
        "*** YOUR CODE HERE ***"
        # my code was taking a long time to run, so I used the np.sum trick from
        # the solution instead of using the ones array
        train_rmse = np.sqrt((np.square(X_train @ W - y_train + b)).mean())
        obj_train.append(train_rmse)
        val_rmse = np.sqrt((np.square(X_val @ W - y_val + b)).mean())
        obj_val.append(val_rmse)
        W_grad = (X_train.T @ X_train @ W + \
              + X_train.T @ (b - y_train) + reg * np.eye(n) @ W) / m_train
        b_grad = (np.sum(X_train @ W) + b * m_train - np.sum(y_train)) / m_train
        W -= W_grad * lr_W
        b -= b_grad * lr_b
        "*** END YOUR CODE HERE ***"
