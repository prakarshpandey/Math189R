...
"""Note to graders:"""
# I had to use the solutions extensively to help figure out this code
# I am not super familiar with numpy, and the softmax lecture was a bit
# all over the place. I'm going to try and do better in future weeks.
...
def NLL(X, y, W, reg=0.0):
    ...
    "*** YOUR CODE HERE ***"
    mu = np.exp(X @ W)
    prob = mu / mu.sum(axis=1).reshape(-1, 1)
    oneHot = y * np.log(prob)
    NLL = -oneHot.sum(axis=1).sum(axis=0) + reg * np.sum(np.diag(W.T @ W))
    "*** END YOUR CODE HERE ***"
    return NLL
...
def grad_softmax(X, y, W, reg=0.0):
    ...
    "*** YOUR CODE HERE ***"
    mu = np.exp(X @ W)
    prob = mu / np.sum(mu, axis=1).reshape(-1, 1)
...
def predict(X, W):
    ...
    "*** YOUR CODE HERE ***"
    mu = np.exp(X @ W)
    probs = mu / mu.sum(axis=1).reshape(-1, 1)
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
    ...
    for reg in lambda_list:
        ...
        "*** YOUR CODE HERE ***"
        W_opt, _ = grad_descent(X_train, y_train_OH, reg=reg, lr=2e-5)
