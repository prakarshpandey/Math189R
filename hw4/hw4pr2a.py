...
def NLL(X, y, W, reg=0.0):
    ...
    "*** YOUR CODE HERE ***"
    # i was getting an overflow error here so i had to check solutions
    nll = -np.sum(np.multiply(y, np.log(sigmoid(X@W))) + np.multiply((1 - y), np.log(1 - sigmoid(X@W)))) + reg / 2 * np.linalg.norm(W) ** 2
    "*** END YOUR CODE HERE ***"
    return nll
...
def newton_step(X, y, W, reg=0.0):
    ...
    # I used the soltuions to check my answer here
    # This material wasn't convered extensively in class
    "*** YOUR CODE HERE ***"
    grad = grad_logreg(X, y, W, reg)
    mu = sigmoid(X@W)
    diagM = np.diag(np.squeeze(np.asarray(np.multiply(mu, 1 - mu))))
    H = X.T @ diagM @ X + reg * np.eye(X.shape[1])
...
def get_description(X, y, W):
    ...
    posPreds = predictions == 1
    negPreds = predictions == 0
    TP = sum(1 == y[posPreds])
    FP = sum(0 == y[posPreds])
    FN = np.sum(1 == y[negPreds])
    precision = TP / (TP + FP)
    recall = TP / (TP + FN)
...
def plot_description(X_train, y_train, X_test, y_test):
    ...
    """
    reg_list = [0.01, 0.03, 0.05, 0.1, 0.3, 0.5, 1, 3, 5, 10, 30]
    a_list = []
    p_list = []
    r_list = []
    f1_list = []
    ...
    "*** YOUR CODE HERE ***"
    for reg in reg_list:
        W_opt, _ = grad_descent(X_train, y_train, reg=reg)
