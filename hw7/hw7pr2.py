...
# I used the solutions to debug my implementation
# Question for grutors: why don't we use sign(X) in the gradient calculation
...
def find_cost(X, y, W , reg):
    ...
    "*** YOUR CODE HERE ***"
    preds = X @ W
    err = float((preds - y).T @ (preds - y))
    return (err + np.sum(np.absolute(reg*W))) / X.shape[0]
    "*** END YOUR CODE HERE ***"
    return cost
...
def prox(X, gamma):
    ...
    # I had no idea what to do here and it wasn't covered in class
    """*** YOUR CODE HERE ***"""
    X[np.abs(X) <= gamma] = 0
    X[X > gamma] -= gamma
    X[X < -gamma] += gamma
...
def grad_lasso(
    ...
    while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
        ...
        X_batch = X[indices, :]
        y_batch = y[indices]
        W_grad = find_grad(X_batch, y_batch, W, reg)
        # i had to look at the solution for this statement as well
        # i do not recall this being discussed in class
        W = prox(W - lr * W_grad, reg * lr)
        cost = find_cost(X_batch, y_batch, W, reg)
...
def lasso_path(X, y, tau_min=1e-8, tau_max=10, num_reg=10):
    ...
    for index in range(num_reg):
        ...
        """grad_lasso(
            X, y, reg=1e6, lr=1e-12, eps=1e-5,
            max_iter=300, batch_size=256, print_freq=1)"""
        W_opt = grad_lasso(X, y, reg=reg, batch_size=1024)[0]
